...
void LoadOdom(const string &strOdomPath, vector<double> &vTimeStamps, vector<Eigen::Vector3d> &vPosition, 
...
int main(int argc, char **argv)
{
    ...
    LoadOdom(string(argv[3]), vTimestampsOdom, vPosition, vOrientation);
    std::cout << "LOAED" << std::endl;
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        // std::cout << "Theta: " << ORB_SLAM2::Converter::normalize_angle(euler[2]) << std::endl;
        //g2o::SE2 odo(x,y,euler[2]);
...
void LoadOdom(const string &strOdomPath, vector<double> &vTimeStamps, vector<Eigen::Vector3d> &vPosition, vector<Eigen::Quaterniond> &vOrientation)
{
    ifstream fOdom;
    fOdom.open(strOdomPath + "/odom.txt");
